...
namespace MobileBank.Forms
{
    public partial class InternetAndTvPayments : Form
    {
        ...
        private void phoneButton_Click(object sender, EventArgs e)
        {
            ...
            if (cardCurrency != "BYN")
            {
                MessageBox.Show("Оплата услуг Интернет и ТВ может производиться только в BYN", "Отмена", MessageBoxButtons.OK, MessageBoxIcon.Information);
                error = true;
            }
            ...
            if (error == false)
            {
                ...
                if (DataStorage.attemps > 0)
                {
                    ...
                    var queryTransaction2 = $"insert into transactions(transaction_type, transaction_destination, transaction_date, transaction_number, transaction_value, id_bank_card) values('Оплата коммунальных услуг', '{InternetComboBox.GetItemText(InternetComboBox.SelectedItem)}', '{formattedTransactionDate}', '{transactionNumber}', '{sum}', (select id_bank_card from bank_card where bank_card_number = '{cardNumber}'))";

                    var queryTransaction3 = $"update clientServices set serviceBalance = serviceBalance + '{sum}' where serviceName = '{InternetComboBox.GetItemText(InternetComboBox.SelectedItem)}' and serviceType = 'Communal'";
