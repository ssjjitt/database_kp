...
namespace MobileBank.Forms
{
    public partial class CharityForm : Form
    {
        ...
        private void phoneButton_Click(object sender, EventArgs e)
        {
            ...
            if (cardCurrency != "BYN")
            {
                MessageBox.Show("Оплата может производиться только в BYN", "Отмена", MessageBoxButtons.OK, MessageBoxIcon.Information);
                error = true;
            }
            ...
            if (error == false)
            {
                ...
                if (DataStorage.attemps > 0)
                {
                    ...
                    var queryTransaction2 = $"insert into transactions(transaction_type, transaction_destination, transaction_date, transaction_number, transaction_value, id_bank_card) values('Благотворительность', '{CharityComboBox.GetItemText(CharityComboBox.SelectedItem)}', '{formattedTransactionDate}', '{transactionNumber}', '{sum}', (select id_bank_card from bank_card where bank_card_number = '{cardNumber}'))";

                    var queryTransaction3 = $"update clientServices set serviceBalance = serviceBalance + '{sum}' where serviceName = '{CharityComboBox.GetItemText(CharityComboBox.SelectedItem)}' and serviceType = 'Communal'";

                    var queryTransaction4 = $"insert into clientPersonalAccount(personal_account, id_service, id_client) values('{PersonalAccountTextBox.Text}', (select id_service from clientServices where serviceName = '{CharityComboBox.GetItemText(CharityComboBox.SelectedItem)}'), '{DataStorage.idClient}')";
        ...
        private void CharityForm_Load(object sender, EventArgs e)
        {
            ...
            CharityComboBox.DataSource = operators;
            CharityComboBox.ValueMember = "id_service";
            CharityComboBox.DisplayMember = "serviceName";
            database.closeConnection();
        }

        private void CharityForm_MouseDown(object sender, MouseEventArgs e)
        {
